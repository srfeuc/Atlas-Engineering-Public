...
The Academy has three layers today and gains a fourth with this vision.
...
**2. Command-Library — the master command reference** (`Command-Library/`). Platform-first (`PowerShell-Tier0` · `Cisco-IOS` · `RouterOS` · `FortiOS` · `Linux`), cross-indexed by service + failure-category. Per-device `Diagnostics.md` quick-refs link **up** into it. **This is where the expanded vision's command content deepens** — see layer 4.

**3. Cert-mapping docs — the certification tracks.** `Atlas-Certification-Lab-Map` (CCNA) · `Atlas-CCNP-Lab-Map` (ENCOR+ENARSI) · `AZ-800-801` · `Atlas-FortiGate-FCP-Lab-Map` · `Atlas-Security-Plus-Domain5` · `CompTIA-Pre-Teardown-Exercise-Catalogue`. (Currency + gaps = backlog **#30**.) 🆕 **A Linux track is being added — anchored to CompTIA Linux+** (operator, 2026-07-31), with the **LPI Linux Essentials** list (the operator's book) as the hands-on **seed** for the Linux section. **Cert-map structure comes from the operator's real exam-book TOCs** — Linux Essentials TOC provided; the **Linux+ track seeds from the O'Reilly CompTIA Linux+ guide** (operator, 2026-07-31 — TOC to come). The map is built from that TOC, anchored to real Atlas Linux hosts (Pi01 · SRV01 · NETBOX01 · BKP01). New track owner = **#30-E**.
...
- A **cert mapping** → the relevant cert lab-map.

## Build sequence (proposed — slices, not one big session)

This is far bigger than one session; build it in slices, each anchored to real Atlas artifacts:

1. **The Linux section first** — seed **received** (the **LPI Linux Essentials** TOC, operator upload 2026-07-31): how to read + configure Linux networking, `journalctl`, the inspection commands for every net fact/service. Deepens `Command-Library/Linux.md`; the **cert track anchors to CompTIA Linux+** (map built from the **O'Reilly CompTIA Linux+ guide** TOC when provided). Anchored to real Atlas Linux hosts (Pi01 · SRV01 · NETBOX01 · BKP01).
2. **The per-appliance troubleshooting guides** — pfSense · FortiGate · MikroTik-as-E-W — each a "how to inspect / troubleshoot a blocked flow" playbook using our real policy sets.
3. **The cross-cutting playbooks** — port conflicts, connection testing, IPS/firewall dropped-traffic diagnosis.
4. **The failure-scenario runbooks** — once the taxonomy above is finalized with the operator.
5. **The README currency fix (#30-A)** — the existing `README.md` "Proposed Curriculum" still teaches retired tech as its primary examples (OpenSSL Lab CA → now AD CS `ADR-0031`; FreeRADIUS → NPS `ADR-0029`; Pi01's old dnscrypt/DoH three-layer → the reduced Pi-hole + chrony). Reconcile to the current build (keep the retired one as an explicit "v1 → v2 / road-not-taken" comparison where it teaches well).
...
- **Next-session brief:** `Labs/Lab-02-Cisco-Core/Operations/Session-23-Claude-Folder-and-Academy-Prompt.md`.
